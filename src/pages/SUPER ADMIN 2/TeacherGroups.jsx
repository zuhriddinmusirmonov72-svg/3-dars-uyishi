...
import { api, groupsAPI, teachersAPI, coursesAPI, roomsAPI, studentsAPI, studentGroupAPI } from '../../api/api';
  ...
  const fetchGroups = async (name = '', max = '', isArchive = false) => {
    ...
    try {
      ...
      if (isArchive) {
        ...
        res = await api.get('/teachers/my/groups');
      }
            ...
            <table className="data-table">
              ...
              <tbody>
                {groups.length === 0 ? (
                  ...
                  groups.map((group) => {
                    const isInactive = activeTab === 1;
                    return (
                    <tr
                      key={group.id}
                      onClick={() => {
                        navigate(`/super-admin-2/groups/${group.id}`);
                      }}
                      style={{
                        cursor: 'pointer',
