...
const TeacherGroupLesson = () => {
  ...
  const loadPage = async () => {
    ...
    try {
      ...
      const [groupRes, studentsRes, lessonRes, attendanceRes, homeworkRes] =
    ...
    try {
      ...
      if (succeeded.length > 0) {
        ...
        setTimeout(() => {
          ...
          navigate(`/groups/${groupId}?tab=0`, { replace: true });
        }, 1000);
      }
    } catch (err) {
  ...
  const handleStudentClick = (student) => {
    ...
    if (!hw || !hw.id) {
      ...
      try {
        navigate(`/groups/${groupId}/homework/${hw.id}/results?tab=1&student=${student.id}`, { replace: false });
      } catch (e) {
  ...
  // ✅ Agar guruh yoki talabalar yuklanmagan bo'lsa
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', padding: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
        <button
          type="button"
          onClick={() => navigate(`/groups/${groupId}?tab=2`)}
          style={{
...
export default TeacherGroupLesson;
